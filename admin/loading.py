...
def _setup_legacy_env():
    importlib.import_module('gan_tracker')
    #print(help(gan_tracker))
    #sys.modules contain a dict which consist of the modules imported to the interpreter. The keys are the name of modules and the values are their directions.
    sys.modules['dlframework'] = sys.modules['gan_tracker']
    sys.modules['dlframework.common'] = sys.modules['gan_tracker']
    for m in ('model_constructor', 'stats', 'settings', 'local'):

        importlib.import_module('gan_tracker.admin.'+m)
        sys.modules['dlframework.common.utils.'+m] = sys.modules['gan_tracker.admin.'+m]
